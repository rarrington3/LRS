module.exports = function (grunt) {
    ...
    grunt.event.on('scaffold.pipe.executed', function () {
        var Context = require('../../tasks/libs/TemplateContext'),
            context = new Context(),
            scaffold = grunt.config('scaffold'),
            settings = grunt.config('settings'),
            path = require('path'),
            name = context.camelCase(scaffold.module).toUpperCase() + '_PIPES',
            indexFile = path.resolve(settings.dir.app, 'modules', scaffold.module, 'pipes', 'index.js'),
        ...
        grunt.log.warn(('A reference to this pipe has been updated in "' + indexFile + '". You may modify this if you need.').yellow);
    });
