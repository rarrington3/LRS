    ...
    function installCordova() {
        ...
        // exec('cordova help', function (error) {
            // if (error) {
                ...
                exec('npm install cordova --save-dev', function (error) {
                    if (error) {
                        ...
                        grunt.log.debug('Installing sync-exec which is needed for Cordova dev...');
                        exec('npm install sync-exec --save-dev', function (error){
                            if (error){
                                grunt.fail.warn('Unable to install sync-exec: ' + error);
                                deferred.reject();
                            } else {
                                grunt.log.debug('sync-exec is now installed.');
                                deferred.resolve();
                            }
                        });
                    }
    ...
    function addPlatform(name) {
        ...
        exec('cordova platform add ' + name, {cwd: settings.dir.cordova}, function (error) {
            if (error) {
                ...
                if (name === 'ios') {
                    exec('npm install ios-sim --save-dev', function (err) {
                        deferred.resolve();
                    });
                }
                else {
