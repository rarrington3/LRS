...
import template from './ReviewLevelLenderResponses.html';
import styles from './ReviewLevelLenderResponses.less';
...
@Component({
    selector: 'review-level-lender-responses',
    template: template,
    styles: [styles]
})
/**
 ...
 */
export default class ReviewLevelLenderResponses {
    ...
    ngOnInit() {
        ...
        let lenderResponseTimeLimitsSource = this._client.resources.admin.reviewLevel.lenderResponseTimeLimits.get().first()
        ...
        Observable.forkJoin(
            ...
            lenderResponseTimeLimitsSource)
            .subscribe(t => {
    ...
    load() {
        this._client.resources.admin.reviewLevel.lenderResponseTimeLimits.get().first()
            .map(reviewLevelTimeLimits => this._mapTimeLimits(reviewLevelTimeLimits))
        ...
        let sources = this._saveQueue.map(entry => {
            switch (entry.operation) {
                case COMMON_OPERATION.UPDATE:
                    return this._client.resources.admin.reviewLevel.lenderResponseTimeLimits
